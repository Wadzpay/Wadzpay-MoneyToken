...
export const useRefundFilterSelectField = ({
  ...
  useEffect(() => {
    serachData()
  }, [filterRefundMode, filterRefundStatus, filterAsset])
  ...
  const serachData = () => {
    ...
    submitFilter({
      ...
      ...(isSearchPOSTxId &&
        searchPOSTxId && { extPosTransactionId: searchPOSTxId }),
      ...(dateFrom && {
        logicalDateFrom: `${dayjs(dateFrom.toString()).format(
          "YYYY-MM-DD"
        )}T00:00:00.000Z`
      }),
      ...(dateTo && {
        logicalDateTo: `${dayjs(dateTo.toString()).format(
          "YYYY-MM-DD"
        )}T23:59:59.999Z`
      }),
      ...
      <div id="filter-data">
        <div className="d-flex flex-row">
          <Button
            ...
            variant="warning"
            onClick={() => serachData()}
            // disabled={extPosTxIDInputRef.current?.value ? true : false}
          >
            Search
          </Button>
          <FilterSearch
          ...
          <Button
            ...
            variant="warning"
            onClick={() => serachData()}
            // disabled={txIdInputRef.current?.value ? true : false}
          >
            Search
          </Button>
          <FilterSearch
            title={"POS Transaction ID"}
            searchDefaultTxt={t("POS Transaction ID")}
            value={searchPOSTxId}
          ...
          <FilterSearch
            title={"Refund POS Transaction ID"}
            searchDefaultTxt={t("Refund POS Transaction ID")}

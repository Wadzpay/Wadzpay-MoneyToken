...
interface EditableCellProps extends React.HTMLAttributes<HTMLElement> {
  ...
  record: ConversionRatesAdjustmentItems
  index: number
...
const ConversionRates: React.FC<Props> = (Props) => {
            ...
            if (value !== undefined && value !== null) {
              ...
              if (parseInt(value) < 0) {
                return Promise.reject(
                  new Error("percentage should be greater than 0.00")
                )
              }
  ...
  useEffect(() => {
    if (data) {
      const markDownList: GetConversionRatesAdjustment = data
      const originData = markDownList.map((element: any, key: number) => {
        const {
          id,
          currencyFrom,
          currencyTo,
  ...
  return (
    <div className="p-2 mt-4 mark-up boxShadow">
        ...
        <div className="table-responsive">
          <Form form={form} component={false}>
            <Table
              ...
              pagination={false}
              locale={{ emptyText: "No mark down found!" }}
            />
          </Form>
        </div>
      </div>
